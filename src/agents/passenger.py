...
class PassengerAgent:
    """Advocates the passenger perspective in the dispute."""
    ...
    def __init__(
        ...
        self.name = "Passenger"
        self.role = (
            "You are a passenger rights advocate analyzing a ride-hailing "
            "dispute. You advocate for the passenger but must remain "
            "evidence-grounded and never assume the passenger is correct."
        )
    ...
    def _evidence_only_result(context: DisputeContext, reasoning: str) -> dict:
        ...
        The agent may still summarise the passenger's available evidence,
        but policy_references must be empty, confidence <= 0.5, and
        requires_human_review must be True.
        """
        evidence: list[str] = []
        if context.description:
            evidence.append(f"Passenger description: {context.description[:200]}")
    ...
    def _fill_defaults(parsed: dict) -> dict:
        ...
        parsed["confidence"] = PassengerAgent._clamp_confidence(parsed["confidence"])
        return parsed
    ...
    async def analyze(self, context: DisputeContext) -> dict:
        ...
        if not policies:
            ...
            return self._evidence_only_result(
                context,
                "No policy clauses were retrieved. The passenger's available "
        ...
        system_prompt = (
            "You are a passenger rights advocate analyzing a ride-hailing "
            "dispute. You advocate for the passenger but must remain "
            "evidence-grounded. Never assume the passenger is correct.\n\n"
            "Rules:\n"
            ...
            "- Distinguish between what the passenger claims and what is verified.\n"
    ...
    async def rebut(self, opponent_argument: str, context: DisputeContext) -> str:
        ...
        system_prompt = (
            "You are a passenger rights advocate writing a rebuttal to the "
            "driver's argument in a ride-hailing dispute.\n\n"
            "Rules:\n"
    ...
    def _rebut_fallback(reason: str) -> str:
        ...
        return (
            "The passenger's rebuttal could not be generated automatically. "
